...
class HyPoGen(nn.Module):
    ...
    def forward_weights(self, ftask):
        weight_dicts = list(
    ...
    def forward(self, ftask, inputs, early_sup=False):
        final_weight_dicts = self.forward_weights(ftask)
        return self.forward_with_weights(inputs, final_weight_dicts, early_sup)
    
    def forward_with_weights(self, inputs, weight_dicts, early_sup=False):
        if early_sup:
            return torch.stack(
                [
                    ...
                    for generated_weight in weight_dicts
                ]
            )
        else:
            return torch.vmap(torch.func.functional_call, in_dims=(None, 0, 0))(
                self.target_net, weight_dicts[-1], inputs
            )

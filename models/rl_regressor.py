...
class HyperRLSolution(nn.Module):
    ...
    def __init__(self, input_param_dim, state_dim, action_dim, embed_dim, hidden_dim,
        ...
        hyper_type = kwargs.pop('hyper_type', 'hyperzero')
        print(f'hyper_type is %s' % hyper_type)
        if hyper_type == 'hyperzero':
    ...
    def forward(self, input_param, state, action, train=False):
        state_action = torch.cat([state, action], dim=-1)
        z, pred_action, q_value = self.hyper_rl_net(input_param, state, state_action, train=train)
    ...
    def forward_weights(self, input_param):
        return self.hyper_rl_net.forward_weights(input_param)

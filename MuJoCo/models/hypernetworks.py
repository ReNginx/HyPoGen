...
class HyPoGenWrapper(HyPoGen):
    ...
    def forward(self, z, base_v, train=False):
        ...
        for i, (opt_block, lr) in enumerate(zip(self.opt_blocks, self.dynamic_lrs)):
            weight_upd_dicts = opt_block(
                ftask, weight_dicts, self.encoders, self.decoders
            )
            weight_upd_dicts = [
                ln(submodule)
            ...
            if train or i == len(self.opt_blocks) - 1:
                ...
                weight_dict = {k: v[rev_indices] for k, v in weight_dict.items()}
        ...
        if train:
            return torch.stack(
                [
                    torch.vmap(torch.func.functional_call, in_dims=(None, 0, 0))(
                        self.target_net, generated_weight, base_v
                    )
            ...
            return torch.vmap(torch.func.functional_call, in_dims=(None, 0, 0))(
                self.target_net, final_weight_dicts[-1], base_v
            )

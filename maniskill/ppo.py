...
if __name__ == "__main__":
    ...
    tmp = make_env(
        args.env_id,
        record_dir=f"logs/{method}/{task_name}/videos",
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=f"./logs/{method}/{task_name}/",
        log_path=f"./logs/{method}/{task_name}/",
    ...
    model = PPO(
        ...
        batch_size=400,
        n_epochs=15,
        tensorboard_log=f"./logs/{method}/{task_name}",
        gamma=0.85,
        target_kl=0.05,
    )
    file_path = f"./logs/{method}/{task_name}/latest_model_" + args.env_id[:-3] + "-our"
    if os.path.exists(file_path + ".zip"):
        model.set_parameters(file_path)
    model.learn(
        args.train_max_steps, callback=[eval_callback, WandbCallback(verbose=2)]
    )
    model.save(file_path)

    # set up model evaluation environment
    eval_env.close()
    record_dir = f"logs/{method}/{task_name}/eval_videos_" + args.env_id[:-3] + "-our"
    eval_env = SubprocVecEnv(
        [
            make_env(
                args.env_id,
    ...
    append_to_file(
        f"logs/{method}/{task_name}/log.txt",
        f"Success Rate: {success_rate}; Episode Lengths: {ep_lens}",
    )

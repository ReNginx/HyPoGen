...
def make_env(env_id, max_episode_steps: int=None, record_dir: str=None, 
    ...
    def _init() -> gym.Env:
        import mani_skill2.envs
        env = gym.make(env_id, obs_mode="state", reward_mode="dense", control_mode=control_mode, cube_half_size=cube_size, agent_stiffness=agent_stiffness, agent_damping=agent_damping, urdf_path=urdf_path)
...
if __name__ == '__main__':
    ...
    if args.exp_name == "":
        args.exp_name = f'{method}_rollout'

    factor = args.length
    if factor != 1.0:
        args.urdf_path = f'ManiSkill2/mani_skill2/assets/descriptions/panda_v2_{factor:.1f}.urdf'
    else:
        args.urdf_path = f'ManiSkill2/mani_skill2/assets/descriptions/panda_v2.urdf'
    

    task_name = args.env_id+f'_cube{args.cube_size:.3f}_stiff{args.agent_stiffness:.0f}_damp{args.agent_damping:.0f}_length{factor:.1f}'
    args.rollout_path = f'logs/{args.exp_name}/{task_name}.npy'
    if os.path.exists(args.rollout_path):
        exit(0)
    ...
    model = PPO.load(path=args.ckpt_path, policy_kwargs=policy_kwargs, verbose=1, 
                     n_steps=args.rollout_steps // args.train_num, batch_size=400, n_epochs=15, 
                     tensorboard_log=f"./logs/{args.exp_name}/{task_name}", gamma=0.85, target_kl=0.05)

    # set up model evaluation environment
    record_dir = f"logs/{args.exp_name}/{task_name}/eval_videos_" + args.env_id[:-3] + "-our"
    eval_env = SubprocVecEnv([make_env(args.env_id, record_dir=record_dir, 
    ...
    returns, ep_lens, traj = evaluate_policy(model, eval_env, deterministic=True, render=False, return_episode_rewards=True, 
                                        n_eval_episodes=args.traj_length, return_traj=True)
    ...
    append_to_file(f"logs/{args.exp_name}/{task_name}/log.txt", f"Success Rate: {success_rate}; Episode Lengths: {ep_lens}")

    np.save(args.rollout_path, traj)
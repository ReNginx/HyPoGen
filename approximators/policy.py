...
class PolicyApproximator:
    ...
    def act_clean(self, env_nums):
        self.prev_action = torch.zeros(env_nums, self.action_dim).to(self.device)
        self.prev_state = torch.zeros(env_nums, self.state_dim).to(self.device)
    ...
    def act_multi(self, input_params, obs):
        ...
        if self.use_pearl:
            ...
            action = self.policy(input_params, obs).cpu().numpy() #mlp
        return action.astype(np.float32)
    ...
    def eval_env(self, env_fn, n_episodes=10, env_params=None, reward_template=None, dynamics_template=None,
        ...
        for env_param in env_params:
            ...
            if input_to_model == 'rew':
                ...
                raise NotImplementedError
            print(reward_config, dynamics_config)
            envs.append(env_fn(reward_config, dynamics_config))
        results = []
        
        if self.use_pearl: 
            self.act_clean(len(envs))

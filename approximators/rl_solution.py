...
class RLApproximator:
    ...
    def __init__(self, model, input_dim, state_dim, action_dim,
        ...
        self.rl_net_optimizer = torch.optim.Adam(
            self.rl_net.parameters()
        , lr=lr)
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.rl_net_optimizer, [1500, 1800, 1900], gamma=0.5)
    ...
    def train(self, training=True):
        ...
        try:   
            self.rl_net.hyper_rl_net.cleanup()
        except:
            pass
    ...
    def act_multi(self, input_params, obs):
        input_params = torch.as_tensor(input_params, dtype=torch.float32, device=self.device).unsqueeze(1)
        obs = torch.as_tensor(obs, dtype=torch.float32, device=self.device)
        task_emb = self.rl_net.embed_task(input_params)
        action = self.rl_net.predict_action(task_emb, obs).cpu().numpy()
        return action.astype(np.float32)
    ...
    def eval_env(
        ...
        for env_param in env_params:
            reward_config = reward_template.copy()
            dynamics_config = dynamics_template.copy()
            if input_to_model == 'rew':
                reward_config['ALL']['margin'] = env_param
            elif input_to_model == 'dyn':
                dynamics_config['length'] = env_param
            elif input_to_model == 'rew_dyn':
                reward_config['ALL']['margin'] = env_param[0]
                dynamics_config['length'] = env_param[1]
            else:
                raise NotImplementedError

            envs.append(env_fn(reward_config, dynamics_config))
        results = []
        for _ in tqdm(range(n_episodes)):
            time_step = [env.reset() for env in envs]
            total_reward = []
            while not time_step[0].last():
                with torch.no_grad():
                    action = self.act_multi(env_params, [ts.observation for ts in time_step])
                    total_reward.append([ts.reward for ts in time_step])

                time_step = [env.step(a) for env, a in zip(envs, action)]
        ...
        result_dict = dict()
        for i, env_param in enumerate(env_params):
            result_dict[mode+'/mean@' + str(env_param)] = mean_reward[i]
            result_dict[mode+'/std@' + str(env_param)] = std_reward[i]

        return result_dict, np.mean(mean_reward)
    ...
    def update(self, data_loader):
        ...
        for batch_idx, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            task_emb, predicted_action, predicted_value = self.rl_net(input_param, state, action, train=True)
            ...
            # max_g =max([p.grad.abs().max() for p in self.rl_net.parameters() if p.grad is not None])
            # print(f'max grad: {max_g:.4f}')

            # torch.nn.utils.clip_grad_norm_(self.rl_net.parameters(), 50.0)
    ...
    def forward_weights(self, input_param):
        return self.rl_net.forward_weights(input_param)
